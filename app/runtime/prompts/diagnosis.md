...
- Grouping gaps are projection gaps too. "grouped concept on <table> has no projected coding child" means the resource's primary coded element (e.g. MedicationRequest.medicationCodeableConcept.coding, Condition.code.coding) is not projected — suggest that coding element. "grouped attribute '<x>' not found on <table>" means that direct element is not projected — suggest the standard FHIR element. "<X> cannot be correlated to <Y> through patient identity" is a relationship limitation, not a single missing field — usually omit it.
